...
const EvidenceList = () => {
  ...
  const handleUpload = async (e) => {
    ...
    try {
      ...
      await evidenceService.upload(uploadData.archivo);
      
      // Refresh evidence list
      const evidenceData = await evidenceService.getByCase(selectedCase);
      setEvidence(evidenceData);
      ...
      setError(Array.isArray(detail) ? detail[0]?.msg : (detail || 'Error al subir evidencia'));
    } finally {
            ...
            >
              ...
              <form onSubmit={handleUpload} className="p-8 space-y-6 max-h-[70vh] overflow-y-auto">
                <div className="space-y-4">
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-gray-400 ml-1 font-bold">Nombre de la Evidencia</label>
                    <input
                      type="text"
                      ...
                      placeholder="Ej: Fotografía de la escena del crimen"
                      required
                    />
                  </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-gray-400 ml-1 font-bold">Archivo</label>
                    <div className="border-2 border-dashed border-white/10 rounded-lg p-8 text-center hover:border-fbi-blue/30 transition-all">
                      ...
                      <label htmlFor="file-upload" className="cursor-pointer">
                        {previewImage ? (
                          <img src={previewImage} className="max-h-40 mx-auto rounded" />
                        ) : (
                          <div className="space-y-2">
                            <Upload className="w-8 h-8 text-gray-600 mx-auto" />
                            <p className="text-gray-500 text-xs uppercase tracking-widest">Click para seleccionar archivo</p>
                          </div>
                        )}
                      </label>
                    </div>
                  ...
                  <div className="space-y-2">
                    <label className="text-[10px] uppercase tracking-widest text-gray-400 ml-1 font-bold">Descripción</label>
                    <textarea
                      rows={3}
                      ...
                      placeholder="Describa la evidencia y su contexto..."
                    />
                  </div>
                </div>
                ...
                <div className="pt-6 border-t border-white/5 flex gap-4">
                  ...
                  >
                    {isUploading ? 'Procesando...' : 'Registrar Evidencia'}
                  </button>
                </div>
              </form>
            </motion.div>
